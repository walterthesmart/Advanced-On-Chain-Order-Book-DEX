    ...
    return (
        <div className="glass-panel" style={{ background: 'rgba(0,0,0,0.2)', padding: '24px', borderRadius: '16px', border: '1px solid var(--glass-border)', marginTop: '20px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '20px', alignItems: 'center' }}>
                <h4 style={{ color: 'var(--text-muted)', margin: 0 }}>Your Open Orders</h4>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                {[...Array(3)].map((_, i) => (
                    <div key={i} className="trade-row" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', fontSize: '13px', padding: '12px 0', borderBottom: '1px solid rgba(255,255,255,0.02)', alignItems: 'center' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                            <div style={{ width: '6px', height: '6px', borderRadius: '50%', background: i % 2 === 0 ? '#00cc66' : '#ff4d4d' }}></div>
                            <span style={{ fontWeight: 600 }}>STX/USDA</span>
                        </div>
                        <span style={{ color: 'var(--text-main)', textAlign: 'center' }}>{(1.45 + i * 0.02).toFixed(2)}</span>
                        <span style={{ color: 'var(--text-main)', textAlign: 'center' }}>{(100 * (i + 1)).toFixed(0)}</span>
                ...
                {/* Empty State visual helper if needed later */}
